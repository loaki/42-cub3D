...
/*   Updated: 2020/01/31 12:06:36 by jfeuilla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_health_color(t_data *data, int x, int y, int id)
{
	...
	(int)((((y - data->res_y / 50) * data->health[id]->height /
	(5 * data->res_y / 50 - data->res_y / 50)) * data->health[id]->width +
	(x - data->res_x / 4) * data->health[id]->width / (3 * data->res_x / 4 -
	data->res_x / 4))) * data->health[id]->bpp / 8));
}
...
void	ft_display_health(t_data *data)
{
	...
	while (id < 2)
	{
		...
		while (y < 5 * data->res_y / 50)
		{
			...
			{
				if (id == 0 || x < ((double)data->life / 100 *
					(3 * data->res_x / 4)))
